...
public class WheelView extends View {
	...
	// ViewConfiguration.getLongPressTimeout() is a bit short (500 ms), so we
	// add a little extra
	private static final long LONG_CLICK_MILLISECONDS_THRESHOLD = ViewConfiguration
			.getLongPressTimeout() + 250;
	...
	private List<OnWheelLongClickedListener> longClickingListeners = new LinkedList<OnWheelLongClickedListener>();
	...
	/**
	 * Adds wheel long clicking listener
	 * 
	 * @param listener
	 *            the listener
	 */
	public void addLongClickingListener(OnWheelLongClickedListener listener) {
		longClickingListeners.add(listener);
	}

	/**
	 * Removes wheel long clicking listener
	 * 
	 * @param listener
	 *            the listener
	 */
	public void removeLongClickingListener(OnWheelLongClickedListener listener) {
		longClickingListeners.remove(listener);
	}
	...
	protected void notifyClickListenersAboutClick(int item) {
		Log.d(TAG, "notifyClickListenersAboutClick");
	...
	/**
	 * Notifies listeners about long clicking
	 */
	protected void notifyLongClickListenersAboutLongClick(int item) {
		Log.d(TAG, "notifyLongClickListenersAboutLongClick");
		for (OnWheelLongClickedListener listener : longClickingListeners) {
			listener.onItemLongClicked(this, item);
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (!isScrollingPerformed) {
				...
				int clickedItem = currentItem + items;
				Log.d(TAG, "clickedItem: " + clickedItem);
				if (isValidItemIndex(clickedItem)) {
					long clickDurationMillis = event.getEventTime()
							- event.getDownTime();
					Log.d(TAG, "clickDurationMillis: " + clickDurationMillis);
					Log.d(TAG, "LONG_CLICK_MILLISECONDS_THRESHOLD: "
							+ LONG_CLICK_MILLISECONDS_THRESHOLD);
					if (clickDurationMillis < LONG_CLICK_MILLISECONDS_THRESHOLD) {
						Log.d(TAG, "normal click");
						notifyClickListenersAboutClick(clickedItem);
					} else {
						Log.d(TAG, "long click");
						notifyLongClickListenersAboutLongClick(clickedItem);
					}
				}
			}
			break;
